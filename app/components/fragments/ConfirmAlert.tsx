...
const ConfirmAlert = ({ validation }: Props) => {
  ...
  return (
    <>
      <div className={`z-50 w-screen fixed items-center h-screen iset-0 left-0 bg-black opacity-80 ${validation === true ? 'flex' : 'hidden'}`}></div>
      <div className={`z-50 w-screen fixed flex items-center h-screen iset-0 left-0 scale-0 ${validation === true && 'scale-100 transition delay-1000'}`}>
